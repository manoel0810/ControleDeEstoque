    ...
    public class DALItensVenda
    {
        ...
        public void Incluir(ModeloItensVenda modelo)
        {
            SqlCommand cmd = new SqlCommand
            {
                ...
                CommandText = "insert into itensvenda(itv_cod,itv_qtde,itv_valor,ven_cod,pro_cod) values (@itv_cod,@itv_qtde,@itv_valor,@ven_cod,@pro_cod);"
            };
        ...
        public void Alterar(ModeloItensVenda modelo)
        {
            SqlCommand cmd = new SqlCommand
            {
                Connection = conexao.ObjetoConexao,
                Transaction = conexao.ObjetoTransacao,
                CommandText = "update itensvenda set itv_qtde = @itv_qtde, itv_valor= @itv_valor " +
        ...
        public void Excluir(ModeloItensVenda modelo)
        {
            SqlCommand cmd = new SqlCommand
            {
                Connection = conexao.ObjetoConexao,
                Transaction = conexao.ObjetoTransacao,
                CommandText = "delete from itensvenda " +
                    "where itv_cod = @itv_cod and ven_cod = @ven_cod and pro_cod = @pro_cod;"
            };
        ...
        public ModeloItensVenda CarregaModeloItensVenda(int ItvCod, int VenCod, int ProCod)
        {
            ...
            {
                Connection = conexao.ObjetoConexao,
                CommandText = "select * from itensvenda where itv_cod = @itv_cod and ven_cod = @ven_cod and pro_cod = @pro_cod;"
            };
